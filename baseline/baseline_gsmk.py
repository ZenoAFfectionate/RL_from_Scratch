...
GSMK_USER_PROMPT_TEMPLATE="""
{question}
Answer:
"""


def format_gsmk_prompt(example):
    """
    Constructs the prompt using ONLY the user template.
    """
    question = example["question"]
    # fill in user prompt template
    full_prompt = GSMK_USER_PROMPT_TEMPLATE.format(
        question=question,
    ).strip()
    return full_prompt
...
if __name__ == "__main__":
    ...
    prompts = [format_gsmk_prompt(example) for example in dataset]
