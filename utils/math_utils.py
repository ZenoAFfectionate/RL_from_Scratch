...
def mathd_normalize_answer(answer: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        # take the inner text if matched
        if m is not None:
...
def _strip_string(string):
    """
    Normalizes a LaTeX math string by removing formatting, units, 
    and standardizing syntax (e.g., fractions, sqrts).
    """
    ...
    def _remove_right_units(string):
        '''removes text explicitly marked as units using \text{}'''
        # "180 \text{ degrees}" -> ['180 ', 'degrees}']
        if "\\text{ " in string:
    ...
    def _fix_sqrt(string):
        r'''converts shorthand \sqrt3 to \sqrt{3}'''
    ...
    # ==========================
    # 1. basic whitespace & formatting cleanup:
    string = string.replace("\n", "")      # remove newlines
    string = string.replace("\\!", "")     # remove LaTeX negative spaces
    string = string.replace("\\\\", "\\")  # find double backslashes

    # 2. matrix standardization
    ...
    string = (  # standardize inequality symbols
        string.replace("\\neq", "\\ne")
        .replace("\\leq", "\\le")
        .replace("\\geq", "\\ge")
    )

    # 4. remove \left and \right
    string = string.replace("\\left", "")
    string = string.replace("\\right", "")

    # 5. Remove unit: miles, dollars if after is not none
    ...
    # 7. remove circ (degrees)
    string = string.replace("^{\\circ}", "")
    string = string.replace("^\\circ", "")
    string = string.replace("\\$", "")

    # 8. remove Currency & Percentages
    string = string.replace("\\$", "")
    ...
    # consider: get rid of e.g. "k = " or "q = " at beginning
    if len(string.split("=")) == 2:
        if len(string.split("=")[0]) <= 2:
            string = string.split("=")[1]

    string = _fix_sqrt(string)  # fix sqrt3 --> sqrt{3}
    ...
    # manually change 0.5 to \frac{1}{2}
    if string == "0.5": string = "\\frac{1}{2}"

    # NOTE: X/Y changed to \frac{X}{Y} in dataset, but in
    # simple cases fix in case the model output is X/Y
...
def normalize_final_answer(final_answer: str) -> str:
    ...
    # Normalize shorthand TeX (e.g. \fracab -> \frac{a}{b})
...
def repeatness(s: str):
    """Calculate a score to determine how repetitive the string is"""
    def ranks(l):
        '''converts a list of values into their sorted rank.'''
    ...
    def lcp(arr, suffixArr, inv_suff):
        '''calculates the Longest Common Prefix between adjacent suffixes'''
        ...
        for ground_truth_elem, given_elem in zip(ground_truth_elems, given_elems):
            if _is_frac(ground_truth_elem) and _is_frac(given_elem):
                ...
                try:
                    ...
                    # Catch any error from sympy and print the inputs
                    print(f"\n[DEBUG] Sympy failed to compare:")
                    print(f"  - Ground Truth: '{ground_truth_elem}'")
                    print(f"  - Given Answer: '{given_elem}'")
                    print(f"  - Error: {e}")
                    is_correct = False # Assume it's incorrect if parsing fails
